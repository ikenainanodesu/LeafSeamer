...
10. [Dashboard UI の開発と受け入れ](#dashboard-ui-の開発と受け入れ)
...
### Dashboard UI の開発と受け入れ

[UI 設計ガイドライン](../../docs/UI_DESIGN_GUIDELINES.md) は、Graphics を除く 12 の Dashboard ページに対する承認済みワークフローを定義します。LeafSeamer Dashboard の表示文言は英語に統一します。

`shared/dashboard-ui/` は UI の権威ソースです。管理対象の各 Dashboard bundle は、バージョン化したローカル `src/dashboard/_leaf-ui/` スナップショットをコミットします。スナップショットを手動で編集せず、権威ソースを更新した後に次を実行します。
