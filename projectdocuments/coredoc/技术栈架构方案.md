...
```mermaid
graph TB
    subgraph Cloud["☁️ 云端层 (Google Cloud)"]
        GS[Google Sheets<br/>冷数据存储]
        GAS[Google Apps Script<br/>表格逻辑处理]
    end

    subgraph Local["💻 本地层 (NodeCG)"]
        NC[NodeCG 核心]

        subgraph Bundles["Bundle 模块"]
            MC[Mixer Control<br/>调音台控制]
            OC[OBS Control<br/>OBS控制]
            GP[Graphics Package<br/>图文包装]
            SM[Schedule Manager<br/>播控列表]
        end
...
├── graphics/
│   ├── lower-third.html      // 下三分之一 (使用 GSAP 动画)
│   ├── scoreboard.html       // 计分板
│   └── components/
│       ├── GameInfo.tsx
│       └── PlayerInfo.tsx
...
| 库          | 版本  | 用途                                 |
| ----------- | ----- | ------------------------------------ |
| GSAP        | 3.12+ | 核心动画引擎,60fps 流畅动画          |
| @gsap/react | 2.1+  | React 集成,useGSAP hook 自动清理动画 |

**GSAP 动画优势**:

- ✅ **性能优异**: GPU 加速,稳定 60fps
- ✅ **Timeline 系统**: 精确控制复杂动画序列
- ✅ **React 18+ 兼容**: 官方 useGSAP hook,自动处理 cleanup
...
```mermaid
flowchart TD
    Start([系统启动]) --> InitNodeCG[初始化 NodeCG]
    InitNodeCG --> LoadBundles[加载所有 Bundles]

    LoadBundles --> InitSync[启动数据同步服务]
    InitSync --> FetchCloud[从 Google Sheets 拉取配置]
    FetchCloud --> CacheData[缓存到本地]

    CacheData --> InitModules{初始化各模块}

    InitModules --> MixerInit[Mixer Control 初始化]
    InitModules --> OBSInit[OBS Control 初始化]
    InitModules --> GraphicsInit[Graphics Package 初始化]
    InitModules --> LoggerInit[Logger System 启动]
    InitModules --> BackupInit[Backup System 启动]

    MixerInit --> MixerConnect{连接调音台}
    MixerConnect -->|成功| MixerListen[监听 OSC 消息]
    MixerConnect -->|失败| MixerRetry[定时重试]
    MixerRetry --> MixerConnect

    MixerListen --> UpdateMixerState[更新 Mixer Replicant<br/>带时间戳]
    UpdateMixerState --> LogEvent1[记录日志事件]

    OBSInit --> OBSConnect{连接 OBS}
    OBSConnect -->|成功| OBSListen[监听 WebSocket 事件]
    OBSConnect -->|失败| OBSRetry[定时重试]
    OBSRetry --> OBSConnect

    OBSListen --> UpdateOBSState[更新 OBS Replicant<br/>带时间戳]
    UpdateOBSState --> LogEvent2[记录日志事件]

    GraphicsInit --> WatchReplicants[监听 Replicants 变化]
    WatchReplicants --> RenderGraphics[渲染图形界面]

    LoggerInit --> ListenAllEvents[监听所有模块事件]
    ListenAllEvents --> WriteLog[异步写入日志<br/>带时间戳]
    WriteLog --> CheckExpired{检查过期日志}
    CheckExpired -->|有过期| CleanLogs[清理过期日志]
    CheckExpired -->|无过期| WaitNext[等待下次检查]

    BackupInit --> CheckSchedule{检查备份计划}
    CheckSchedule -->|到达时间| CreateBackup[创建备份<br/>带时间戳]
    CreateBackup --> CleanOldBackup[清理过期备份]
    CleanOldBackup --> WaitNextBackup[等待下次备份]

    UpdateMixerState --> Dashboard1[Dashboard 显示]
    UpdateOBSState --> Dashboard2[Dashboard 显示]

    Dashboard1 --> UserAction1{用户操作}
    UserAction1 -->|修改配置| SendOSC[发送 OSC 命令]
    SendOSC --> MixerListen
    UserAction1 -->|上行同步| SyncToCloud[同步到 Google Sheets]

    Dashboard2 --> UserAction2{用户操作}
    UserAction2 -->|切换场景| SendWS[发送 WebSocket 命令]
    SendWS --> OBSListen

    SyncToCloud --> UpdateSheet[更新 Google Sheets]
    UpdateSheet --> LogEvent3[记录同步日志]
```
...
sequenceDiagram
    ...
    User->>Ext: 点击 "连接调音台"
    Ext->>OSC: 创建 OSC 连接
    OSC->>Mixer: 连接到 IP:Port
    Mixer-->>OSC: 连接成功
    OSC-->>Ext: 触发 connected 事件
    Ext->>Rep: 更新 connected = true<br/>(timestamp: now)
    Ext->>Log: 记录连接成功日志
    Log-->>Log: 异步写入日志文件<br/>(带时间戳)

    loop 实时监听
        Mixer->>OSC: 发送状态变化 (OSC 消息)
        OSC->>Ext: 解析 OSC 消息
        Ext->>Ext: 更新内部状态
        Ext->>Rep: 更新 Replicant<br/>(timestamp: now)
        Rep-->>User: 自动同步到 Dashboard
        Ext->>Log: 记录状态变化日志
    end
...
sequenceDiagram
    ...
    User->>Ext: 点击 "连接 OBS"
    Ext->>WS: 建立 WebSocket 连接
    WS->>OBS: ws://localhost:4455
    OBS-->>WS: 连接成功 + 鉴权
    WS-->>Ext: 触发 connected 事件
    Ext->>Rep: 更新 connected = true<br/>(timestamp: now)
    Ext->>Log: 记录连接成功日志

    Ext->>OBS: 请求当前状态
    OBS-->>Ext: 返回场景、推流状态等
    Ext->>Rep: 更新 OBS Replicant<br/>(timestamp: now)

    loop 实时监听
        OBS->>WS: 推流状态事件 (码率、丢包率)
        WS->>Ext: 解析事件
        Ext->>Rep: 更新 streamStats<br/>(timestamp: now)
        Rep-->>User: 同步到 Dashboard 显示
        Ext->>Log: 记录状态日志
    end

    User->>Ext: 添加视频到队列
    Ext->>Queue: 添加到 videoQueue
    Queue->>Rep: 更新 Replicant<br/>(timestamp: now)
    Rep-->>User: 显示队列
...
```mermaid
flowchart TD
    Start([备份系统启动]) --> LoadConfig[加载备份配置]
    LoadConfig --> CheckMode{备份模式}

    CheckMode -->|自动定时备份| SetSchedule[设置定时任务<br/>Node-cron]
    CheckMode -->|手动备份| WaitManual[等待用户触发]

    SetSchedule --> WaitTrigger[等待定时触发]
    WaitTrigger -->|到达时间| StartBackup
    WaitManual -->|用户点击| StartBackup[开始备份]

    StartBackup --> CollectData[收集所有 Replicants 数据]
    CollectData --> CollectConfig[收集配置文件]
    CollectConfig --> CreateTimestamp[生成时间戳]
    CreateTimestamp --> CompressData[压缩数据<br/>使用 zlib]

    CompressData --> SaveBackup[保存备份文件<br/>backup_YYYYMMDD_HHmmss.zip]
    SaveBackup --> LogBackup[记录备份日志]

    LogBackup --> CheckOldBackups{检查旧备份}
    CheckOldBackups -->|超过保留期限| DeleteOld[删除过期备份]
    CheckOldBackups -->|未超期| KeepOld[保留备份]

    DeleteOld --> LogDelete[记录删除日志]
    LogDelete --> Done([备份完成])
    KeepOld --> Done
    ...
    %% 恢复流程
    UserRestore[用户选择恢复] --> ConfirmDialog{确认弹窗}
    ConfirmDialog -->|取消| CancelRestore([取消恢复])
    ConfirmDialog -->|确认| LoadBackupFile[加载备份文件]

    LoadBackupFile --> DecompressData[解压数据]
    DecompressData --> ValidateData{验证数据完整性}
    ValidateData -->|失败| ShowError[显示错误提示]
    ValidateData -->|成功| RestoreReplicants[恢复 Replicants]

    RestoreReplicants --> RestoreConfig[恢复配置文件]
    RestoreConfig --> LogRestore[记录恢复日志]
    LogRestore --> NotifyModules[通知所有模块重载]
    NotifyModules --> RestoreDone([恢复完成])
```
