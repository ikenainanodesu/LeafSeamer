...
- 另保留 16 个行为与基础设施回归：4 个核心业务流程（Schedule 外部条目只读、Schedule 删除后的焦点恢复、Seamer tabs/card/trigger 合同、Backup L3 pending/error/success）、2 个测试服务器访问边界/空闲持续可用性检查，以及 10 个 ATEM、OBS、VB 设备命令与焦点流程。14 个挂载 Dashboard 的业务、设备和焦点流程检查无运行时错误；2 个服务器流程直接检查 HTTP 访问边界与持续可用性。
- 新增或修改页面时，先运行 `npm run ui:check`，再运行 `npm run test:server` 和 `npm run test:ui`。只有经过评审的有意视觉变化需要更新基线时，才运行 `npm run test:ui:update`。
...
- [ ] 已执行 `npm run ui:check`、`npm run test:server`、`npm run test:ui`；有意刷新视觉基线时才执行 `npm run test:ui:update`。
